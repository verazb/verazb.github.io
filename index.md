...
- Unemployment Insurance with Response Heterogeneity (with [Conny Wunsch](https://sites.google.com/view/cwunsch)). --- draft available upon request
  ...
  In many publicly-funded unemployment insurance (UI) systems, the generosity of coverage increases with age conditional on time employed. This paper examines how the responses to UI and the implied insurance-incentive trade-off vary with these two characteristics. We estimate a comprehensive set of duration and wage elasticities using multiple discontinuities in coverage duration in Germany. The findings reveal that age is associated with a larger negative fiscal externality due to transfer costs, which is not fully offset by a higher insurance value. Workers with longer contribution generate lower costs as moral hazard responses are smaller, with the insurance value varying little. Both characteristics thus carry relevant information on labor supply behavior, which justifies using them for welfare-enhancing policy differentiation. Lowering coverage duration for older workers and increasing it for prime-age workers with stable employment could increase welfare.
